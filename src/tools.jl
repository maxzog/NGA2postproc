...
function Lag4Interp(field::grid, xiv::Vector{Float32})::Vector{Float32}   
  ...
  # end
   ...
   xv = LinRange(field.Δ/2, field.L - field.Δ/2, field.n)
   rvec = zeros(Float32, 3)
   for i in 1:4, j in 1:4, k in 1:4
      ii = n-2+i
      jj = p-2+j
...
function Lag4InterpFiltered(field::grid, xiv::Vector{Float32})::Vector{Float32}   
  # for i in eachindex(xiv)
  #    if xiv[i] > field.L
  #       xiv[i] = xiv[i] - field.L
  #    end
  # end
   ...
   for i in 1:4, j in 1:4, k in 1:4
      ...
      rvec[1] += field.Uf[ii, jj, kk] * lx * ly * lz
      rvec[2] += field.Vf[ii, jj, kk] * lx * ly * lz
      rvec[3] += field.Wf[ii, jj, kk] * lx * ly * lz
   end
   return rvec
end

function Lag4InterpSubFilter(field::grid, xiv::Vector{Float32})::Vector{Float32}   
  # for i in eachindex(xiv)
  #    if xiv[i] > field.L
  #       xiv[i] = xiv[i] - field.L
  #    end
  # end
   ...
   for i in 1:4, j in 1:4, k in 1:4
      ...
      rvec[1] += (field.U[ii, jj, kk] - field.Uf[ii, jj, kk]) * lx * ly * lz
      rvec[2] += (field.V[ii, jj, kk] - field.Vf[ii, jj, kk]) * lx * ly * lz
      rvec[3] += (field.W[ii, jj, kk] - field.Wf[ii, jj, kk]) * lx * ly * lz
   end
   return rvec
end
